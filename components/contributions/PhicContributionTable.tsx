...
export function PhicContributionTable() {
  ...
  return (
    <ReportPageShell
      title="PHIC Contribution Report"
      description="PhilHealth contributions for the selected year."
      filters={
      ...
      {query.isLoading ? (
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {rows.map((row, i) => (
                <TableRow key={`${row.pyh_code}-${row.emp_philhealth}-${i}`}>
                  <TableCell>{row.pyh_desc}</TableCell>
                  <TableCell>{row.emp_name}</TableCell>
                  <TableCell>{row.emp_philhealth}</TableCell>
                  <TableCell className="text-right">{fmt(row.salary)}</TableCell>
                  <TableCell className="text-right">{fmt(row.pyd_phic)}</TableCell>
                  <TableCell className="text-right">{fmt(row.pyd_phicer)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
