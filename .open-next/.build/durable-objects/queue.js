...
// node_modules/.pnpm/@opennextjs+cloudflare@1.16.2_next@16.1.5_@babel+core@7.29.0_react-dom@19.2.3_react@19._957d2f7741d9740c994748cab8b329ad/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "4547bd4ec08ac1a28ddbac822756abf7",
          "x-isr": "1"
        },
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "MASTam1n7qZ-ERsXRvWPV");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "MASTam1n7qZ-ERsXRvWPV");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "MASTam1n7qZ-ERsXRvWPV");
